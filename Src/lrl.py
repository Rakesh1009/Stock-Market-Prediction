...
class lrl(nn.Module):
  ...
  def forward(self,x):
    ...
    b = self.relu(a)
    #print(b.shape)
    out = self.linear_2(b[:, -1, :])
    #print(out.shape)
    return out


model = lrl(input_dim=input_dim, hidden_dim=hidden_dim, output_dim=output_dim, num_layers=num_layers)
...
plt.title(company+' Stock Price Prediction Using Linear-ReLu-Linear'+'      Test Score: %.2f RMSE' % (testScore))
plt.xlabel('Time')
plt.ylabel(company + ' Stock Price LRL')
plt.legend()
plt.savefig('./../Output/'+company+'_pred_lrl.png')
plt.show()
